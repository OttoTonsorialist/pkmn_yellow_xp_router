...
class YellowRecorder(route_recording.recorder.RecorderGameHookClient):
    ...
    def on_mapper_loaded(self):
        ...
        if self._controller.is_ready():
            ...
            # try to find the right mapper version
            gh_gen_one_const.use_new_mapper()
            invalid_props = self.validate_constants(gh_gen_one_const)
            if invalid_props:
                gh_gen_one_const.use_old_mapper()
                invalid_props = self.validate_constants(gh_gen_one_const)
                if invalid_props:
